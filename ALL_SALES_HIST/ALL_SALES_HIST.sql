/*
Primary Difference: The Source Table
The most significant difference is the table used in the FROM clause.

ALL_SALES_Hist_11-21-25.sql (Old): Sources data from the Bronze layer.
    ```FROM "BRONZE_DATA"."TCM_BRONZE"."SALES_TABLE" sls```

ALL_SALES_HIST.sql (New): Sources data from the Gold layer.
    ```FROM  GOLD_DATA.TCM_GOLD.BOOKINGS_INVOICE_REPORTING sls```

Potential Reason:
This represents an architectural promotion. 
The view is being repointed from a raw/ingestion table (SALES_TABLE) to a curated/aggregated table (BOOKINGS_INVOICE_REPORTING). 
The Gold table likely contains cleaner data, consolidated logic (combining bookings and invoices), or calculated fields that the Bronze table lacks.
*/
create or replace view BRONZE_DATA.TCM_BRONZE.ALL_SALES_HIST(
	...
	"CALENDAR DATE",
	"Booking Type Table"
) COMMENT='Changed from ALL_SALES_HIST_TEST to ALL_SALES_HIST'
 as
-- ================SELECT Statement ========================
SELECT
