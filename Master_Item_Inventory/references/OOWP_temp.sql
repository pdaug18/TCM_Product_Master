/*
This report provides a comprehensive view of open sales orders with a focus on their 
    - fulfillment status, 
    - particularly 
in relation to the manufacturing process.

* Primary Business Problem: Order Fulfillment and Production Tracking
    The main purpose of this report is to give a detailed, real-time status update on all open customer orders. 
    It answers critical questions for sales, customer service, and production planning teams:

    1. What is the current status of our open orders?
        It shows which orders are acknowledged (flag_ack), partially picked (flag_pick), or have credit holds (CREDIT_STATUS).
        It calculates the open quantity (qty_open) that still needs to be shipped.
    
    2. Where is this order in the production lifecycle?
        This is a key function. The report links sales orders to internal shop orders.
        It shows if the required items are Released to Production (Qty_Rel) or if they have Started Manufacturing (Qty_Cut).
        It provides a STAT_REC_SO_display to give a clear, user-friendly status of the shop order (e.g., 'R' for Released, 'W' for Waiting, 'D' for Done).

    3. When can we expect this order to be ready?
        The report calculates DATE_CALC_START and DATE_CALC_END, which seem to be projected start and completion dates for fulfillment, based on a set of business rules 
        (e.g., customer-specific lead times, stock status).
        It also tracks how long an order has been waiting since it was last picked (WorkingDaysSinceLastPicked), highlighting potential bottlenecks.

    4. How are we sourcing the items for this order?
        It determines if an item is Stocked (1-STOCK), Made-to-Order (2-MTO), or a raw material/fabric (3-FABRIC).
        This helps in understanding the fulfillment path and expected lead times.

* Secondary Business Problems:
    1. Inventory Visibility: 
        It provides inventory details like quantity on hand (QTY_ONHD), allocated (QTY_ALLOC), and on order (QTY_ONORD) for the specific items and locations tied to the open orders.
    2. Customer and Sales Rep Insights: 
        It includes customer information (cust_soldto, name_cust_soldto), their PO number (id_po_cust), and the assigned sales representative (id_slsrep), making it a useful tool for sales and account managers to track their orders.
    3. Operational Efficiency Monitoring: 
        It tracks metrics like WorkingDaysSinceLastPicked and the status of shop orders, allowing managers to identify delays in the fulfillment process, from picking to production and shipping.


In essence, this report acts as a central dashboard for managing the entire order-to-delivery process, 
providing visibility into both the sales order status and the underlying manufacturing operations required to fulfill it.
